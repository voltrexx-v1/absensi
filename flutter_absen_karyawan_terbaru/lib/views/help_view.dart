...
class _HelpViewState extends State<HelpView> {
  ...
  void initState() {
    ...
    _fetchConfigs();
    _loadTickets();
    // Polling tiket secara realtime di background setiap 3 detik agar karyawan langsung lihat saat tiket selesai
  ...
  Future<void> _submitReply() async {
    ...
    bool success = await ApiService.updateTicket(_replyingTo!['id'].toString(),
        {'status': 'Closed', 'reply': _replyMsg});
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text("Tiket berhasil dibalas & diselesaikan!"),
            backgroundColor: AppColors.emerald500));
        setState(() {
          // Secara instan update tiket di UI sebelum request background _loadTickets selesai
          int idx = _allTickets.indexWhere((t) => t['id'].toString() == _replyingTo!['id'].toString());
          if (idx != -1) {
            _allTickets[idx]['status'] = 'Closed';
            _allTickets[idx]['reply'] = _replyMsg;
            _allTickets[idx]['balasan'] = _replyMsg;
            _allTickets[idx]['updated_at'] = DateTime.now().toIso8601String();
          }
  ...
  Future<void> _deleteSelectedTickets() async {
    ...
    bool? confirm = await showDialog<bool>(
        ...
        builder: (c) => AlertDialog(
              ...
              content: Text(
                  "Apakah Anda yakin ingin menghapus ${_selectedTicketIds.length} tiket keluhan yang dipilih? Aksi ini tidak dapat dibatalkan.",
  ...
  Future<void> _submitTicket() async {
    if (_subject.isEmpty || _message.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
          content: Text("Harap lengkapi Subjek dan Pesan!"),
          backgroundColor: AppColors.rose500));
      return;
    }
    ...
    if (mounted) {
      if (success) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content:
                Text("Tiket berhasil dikirim. Admin akan segera merespons."),
            backgroundColor: AppColors.emerald500));
        setState(() {
          _subject = '';
          _message = '';
        ...
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(
            content: Text("Gagal mengirim tiket."),
            backgroundColor: AppColors.rose500));
      }
    }
  ...
  Widget _buildAdminView(List<Map<String, dynamic>> allTickets) {
    ...
    if (_selectedAreaFilter != 'Semua Area') {
      allTickets = allTickets
          .where((t) => (t['area'] ?? '') == _selectedAreaFilter)
          .toList();
    }

    List<Map<String, dynamic>> openTickets =
        allTickets.where((t) => t['status'] == 'Open').toList();
    List<Map<String, dynamic>> closedTickets =
        allTickets.where((t) => t['status'] == 'Selesai').toList();
    ...
    List<Map<String, dynamic>> displayedTickets =
        _adminTab == 'Open' ? openTickets : closedTickets;
                    ...
                    children: [
                      IconButton(
                        icon: const Icon(Icons.arrow_back,
                            color: AppColors.slate600, size: 24),
                    ...
                    children: [
                      ...
                      Container(
                        ...
                        child: DropdownButtonHideUnderline(
                          child: DropdownButton<String>(
                            value: _availableAreas.contains(_selectedAreaFilter)
                                ? _selectedAreaFilter
                                : (_availableAreas.isNotEmpty
                                    ? _availableAreas.first
                                    : null),
                            ...
                            onChanged: (String? newValue) {
                              setState(() {
                                ...
                                _selectedTicketIds
                                    .clear(); // Bersihkan centang saat ganti area
                              });
                            },
                      ...
                      Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            _buildAdminTabBtn(
                                "MENUNGGU (${openTickets.length})", 'Open'),
                            _buildAdminTabBtn(
                                "SELESAI (${closedTickets.length})", 'Selesai'),
                          ],
                        ),
                      ),
                    ],
                        ...
                        children: [
                          ...
                          if (_selectedTicketIds.isNotEmpty)
                            ElevatedButton.icon(
                              ...
                              onPressed:
                                  _isDeleting ? null : _deleteSelectedTickets,
                              icon: _isDeleting
                                  ? const SizedBox(
                                      width: 14,
                                      height: 14,
                              ...
                              label: Text(
                                  "HAPUS (${_selectedTicketIds.length})",
                                  style: const TextStyle(
                                      fontWeight: FontWeight.w900,
                                      fontSize: 10,
                                      letterSpacing: 1)),
                        ...
                        child: ConstrainedBox(
                          ...
                          child: DataTable(
                            ...
                            onSelectAll: (val) {
                              setState(() {
                                if (val == true) {
                                  _selectedTicketIds.addAll(displayedTickets
                                      .map((t) => t['id'].toString())
                                      .where((id) =>
                                          !_selectedTicketIds.contains(id)));
                                } else {
                                  _selectedTicketIds.clear();
                                }
                              });
                            ...
                            rows: displayedTickets.map((t) {
                              ...
                              return DataRow(
                                  ...
                                  selected:
                                      _selectedTicketIds.contains(t['id']),
                                  onSelectChanged: (val) {
                                    setState(() {
                                      if (val == true) {
                                  ...
                                  cells: [
                                    DataCell(Text(_formatDate(t['created_at']),
                                        style: const TextStyle(
                                            fontSize: 10,
                    ...
                    if (displayedTickets.isEmpty)
                      Padding(
                        ...
                        child: Center(
                          child: Text(
                              _adminTab == 'Open'
                                  ? "TIDAK ADA TIKET YANG MENUNGGU SAAT INI."
            ...
            child: Column(
              ...
              children: [
                Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    const Text("BALAS TIKET KELUHAN",
                        style: TextStyle(
                            fontSize: 20,
                            fontWeight: FontWeight.w900,
                            color: AppColors.slate800,
                        ...
                        children: [
                          Text(
                              ...
                              style: const TextStyle(
                                  ...
                                  color: AppColors.slate500,
                                  letterSpacing: 1)),
                          Text(_formatDate(_replyingTo!['created_at']),
                              style: const TextStyle(
                                  fontSize: 9,
                                  fontWeight: FontWeight.bold,
                                  color: AppColors.slate400,
                ...
                // Form Balasan
                const Text("PESAN BALASAN & SOLUSI",
                    style: TextStyle(
                        fontSize: 9,
                        fontWeight: FontWeight.w900,
                        color: AppColors.slate400,
  ...
  Widget _buildEmployeeView(List<Map<String, dynamic>> allTickets) {
    List<Map<String, dynamic>> myTickets =
        allTickets.where((t) => t['user_id'].toString() == widget.user.id.toString()).toList();
    myTickets.sort(
        (a, b) => (b['created_at'] ?? '').compareTo(a['created_at'] ?? ''));
                          ...
                          children: [
                            Container(
                              ...
                              child: const Icon(Icons.phone_in_talk,
                                  color: AppColors.slate900, size: 24),
                            ),
                            const SizedBox(height: 24),
                            const Text("LAYANAN CS",
                                style: TextStyle(
                                    fontSize: 20,
                                    fontWeight: FontWeight.w900,
                                    color: Colors.white)),
                            const SizedBox(height: 8),
                            const Text(
                                "Hubungi administrator jika Anda mengalami kendala mendesak terkait sistem.",
                                style: TextStyle(
                                    fontSize: 11,
                            ...
                            Container(
                              ...
                              child: const Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 8),
                                  Text(
                                      "+62 812-3456-7890 dan +62 851-9165-1651",
                                      style: TextStyle(
                                          fontSize: 16,
                                          fontWeight: FontWeight.w900,
                      ...
                      Container(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 24),
                            _buildFaqItem(
                                "GAGAL ABSEN LOKASI / GPS?",
                                "Pastikan izin lokasi GPS di browser atau HP Anda aktif. Jika peta tidak muncul, klik tombol \"Deteksi Ulang\".",
                                AppColors.yellow500),
                            const SizedBox(height: 16),
                            _buildFaqItem(
                                "LUPA ABSEN / SUSULAN?",
                                "Gunakan form \"Buat Tiket Keluhan Baru\" di samping untuk melapor kepada Admin IT agar absensi susulan dapat diinputkan ke dalam sistem.",
                                AppColors.blue500),
                            const SizedBox(height: 16),
                            _buildFaqItem(
                                "ABSEN PERJALANAN DINAS",
                                "Gunakan status \"Perjalanan Dinas\" pada menu Kehadiran. Sistem akan melacak alamat lokasi Anda secara otomatis tanpa dibatasi radius jarak.",
                                AppColors.indigo500),
                            const SizedBox(height: 16),
                            _buildFaqItem(
                                "JAM KERJA TERLAMBAT",
                                "Sistem secara otomatis menandai \"Terlambat\" jika Anda absen melebihi batas masuk pada shift kerja Anda.",
                                AppColors.emerald500),
                          ],
                        ),
                      )
                        ...
                        child: Column(
                          ...
                          children: [
                            const Row(
                              children: [
                                Icon(Icons.send,
                                    size: 18, color: AppColors.yellow500),
                                SizedBox(width: 8),
                                Text("BUAT TIKET KELUHAN BARU",
                                    style: TextStyle(
                                        fontSize: 12,
                                        fontWeight: FontWeight.w900,
                                        color: AppColors.slate900,
                            ...
                            const SizedBox(height: 24),
                            const Text("DETAIL PESAN / ALASAN",
                                style: TextStyle(
                                    fontSize: 9,
                                    fontWeight: FontWeight.w900,
                                    color: AppColors.slate400,
                              ...
                              children: [
                                ...
                                Container(
                                  ...
                                  decoration: BoxDecoration(
                                      ...
                                      borderRadius: BorderRadius.circular(12)),
                                  child: Text("${myTickets.length} TIKET",
                                      style: const TextStyle(
                                          fontSize: 9,
                                          fontWeight: FontWeight.w900,
                                          color: AppColors.slate400)),
                                ...
                                child: Center(
                                  child: Column(
                                    children: [
                                      ...
                                      SizedBox(height: 16),
                                      Text("BELUM ADA RIWAYAT TIKET KELUHAN.",
                                          style: TextStyle(
                                              fontSize: 10,
                                              fontWeight: FontWeight.w900,
