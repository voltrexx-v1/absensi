...
class _LoginScreenState extends State<LoginScreen> {
  ...
  final TextEditingController _regConfirmPassController = TextEditingController(); // TAMBAHAN KONFIRMASI
    ...
    try {
      ...
      if (kIsWeb) {
        ...
        if (Platform.isAndroid) {
          ...
          hardwareId = iosInfo.identifierForVendor ?? ""; // IDFV iOS (Aman privasi)
      ...
      builder: (context) {
        return StatefulBuilder(
          builder: (context, setDialogState) {
            Widget _buildRowInput(String label, TextEditingController controller, {bool isObscure = false, String hint = ""}) {
               return Padding(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Column(
                        ...
                        children: [
                          _buildRowInput("Alamat Email", emailCtrl, hint: "email@contoh.com"),
                          _buildRowInput("Password Baru", newPassCtrl, isObscure: true, hint: "Sandi Baru"),
                          _buildRowInput("Konfirmasi Password", confirmPassCtrl, isObscure: true, hint: "Ulangi Sandi"),
                                ...
                                child: ElevatedButton(
                                  ...
                                  onPressed: isSending ? null : () async {
                                      ...
                                      try {
                                        ...
                                        var userMatch = users.where((u) => u['email'] == emailCtrl.text.trim()).toList();
                                        
                                        if (userMatch.isNotEmpty) {
                                          var userDoc = userMatch.first;
                                          await ApiService.updateUser(userDoc['id'].toString(), {
                                            'password': newPass
                                          });
    ...
    try {
      ...
      if (apiUser != null) {
          ...
          if ((_mode == 'karyawan' && apiUser.role != 'Karyawan') || 
             ...
             setState(() => _errorMsg = "Gagal masuk: Akun Anda tidak terdaftar sebagai ${_mode == 'admin' ? 'Admin' : _mode == 'head' ? 'IT' : 'Karyawan'}");
             return;
          }
          // [SINKRONISASI AREA PENUGASAN]
          // Jika karyawan memilih area yang berbeda di halaman login, terapkan area tersebut
          if (_mode == 'karyawan' && _selectedArea.isNotEmpty && _selectedArea != apiUser.area) {
             apiUser = UserModel(
               id: apiUser.id,
        ...
        setState(() => _errorMsg = "NRP/ID tidak ditemukan atau password salah.");
      }
    } catch (e) {
  ...
  Future<void> _handleRegisterKaryawan() async {
    ...
    if (nama.isEmpty || nik.isEmpty || pass.isEmpty || confirmPass.isEmpty || email.isEmpty || _regTanggalLahir == null) {
      setState(() => _errorMsg = "Harap isi Nama, NRP, Email, Tanggal Lahir, dan Kata Sandi lengkap!");
      return;
    }
    ...
    try {
      ...
      if (response['success'] != true) {
        setState(() => _errorMsg = response['data']?['message'] ?? "Gagal mendaftarkan akun.");
        return;
      }
  ...
  Widget _buildDesktopLoginForm() {
    ...
    return Column(
      ...
      children: [
        ...
        if (_step == 'select') ...[
          _buildDesktopSelectionButton('karyawan', 'KARYAWAN', Icons.person, AppColors.yellow500),
          const SizedBox(height: 16),
          _buildDesktopSelectionButton('admin', 'IT SUPPORT', Icons.security, AppColors.rose500),
        ] else ...[
          Row(
            children: [
              IconButton(
                icon: const Icon(Icons.arrow_back, color: Colors.white54),
                onPressed: () => setState(() { _step = 'select'; _errorMsg = ''; }),
              ...
              Expanded(
                child: Container(
                  ...
                  child: Row(
                    children: [
                      Icon(
                        _mode == 'karyawan' ? Icons.person : (_mode == 'head' ? Icons.supervisor_account : Icons.security),
                        color: _mode == 'karyawan' ? AppColors.yellow500 : (_mode == 'head' ? AppColors.blue500 : AppColors.rose500),
                        size: 16,
                      ),
                      const SizedBox(width: 12),
                      Text("Login ${_mode == 'head' ? 'ADMIN AREA' : _mode == 'admin' ? 'IT SUPPORT' : 'KARYAWAN'}", style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 12, letterSpacing: 1)),
                    ],
                  ),
                )
              )
          ...
          if (_mode == 'karyawan') ...[
            ...
            DropdownButtonFormField<String>(
              ...
              initialValue: _availableAreas.contains(_selectedArea) ? _selectedArea : (_availableAreas.isNotEmpty ? _availableAreas.first : null),
          ...
          _buildDesktopInputLabel(_mode == 'head' ? "ID ADMIN AREA" : _mode == 'admin' ? "ID ADMINISTRATOR" : "NRP KARYAWAN"),
          LayoutBuilder(
            builder: (context, constraints) => RawAutocomplete<String>(
              ...
              fieldViewBuilder: (c, t, f, o) {
                return TextField(
                  ...
                  decoration: _desktopInputDeco("Masukkan ${_mode == 'head' ? 'Admin Area' : _mode == 'admin' ? 'IT Support' : 'Karyawan'} ID/NRP"),
                );
              },
          ...
          TextField(
            ...
            decoration: _desktopInputDeco("Masukkan Kata Sandi").copyWith(
              suffixIcon: IconButton(
                icon: Icon(_showLoginPassword ? Icons.visibility : Icons.visibility_off, color: Colors.white38, size: 18),
          ...
          SizedBox(
            ...
            child: ElevatedButton(
              style: ElevatedButton.styleFrom(
                backgroundColor: _mode == 'karyawan' ? AppColors.yellow500 : (_mode == 'head' ? AppColors.blue500 : AppColors.rose500),
              ...
              child: Row(
                mainAxisAlignment: MainAxisAlignment.center,
                children: [
                  Text("MASUK SEKARANG", style: TextStyle(color: _mode == 'karyawan' ? AppColors.slate900 : Colors.white, fontWeight: FontWeight.w900, letterSpacing: 2, fontSize: 12)),
                  const SizedBox(width: 8),
                  Icon(Icons.arrow_forward, color: _mode == 'karyawan' ? AppColors.slate900 : Colors.white, size: 16)
                ],
              ),
            ),
          ),
          ...
          if (_mode == 'karyawan' || _mode == 'head') ...[
             ...
             TextButton(
                ...
                child: Text("DAFTAR SEBAGAI ${_mode == 'head' ? 'ADMIN AREA' : 'KARYAWAN BARU'}", style: const TextStyle(color: Colors.white54, fontWeight: FontWeight.w900, fontSize: 10, letterSpacing: 1, decoration: TextDecoration.underline))
             )
          ]
        ]
      ],
  ...
  Widget _buildDesktopSelectionButton(String modeValue, String label, IconData icon, Color iconColor) {
    return InkWell(
      onTap: () {
        setState(() {
          _mode = modeValue;
  ...
  Widget _buildDesktopRegisterForm() {
    return Column(
      ...
      children: [
        ...
        Text(_mode == 'head' ? "Registrasi Admin Area" : "Registrasi Karyawan", style: const TextStyle(color: Colors.white, fontSize: 24, fontWeight: FontWeight.w900, letterSpacing: -0.5)),
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 16),
            Expanded(child: _buildDesktopInputBlock("NO HANDPHONE", TextField(textInputAction: TextInputAction.next, keyboardType: TextInputType.phone, controller: _regKontakController, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13), decoration: _desktopInputDeco("No HP Aktif")))),
          ],
        ),
        const SizedBox(height: 16),
        ...
        Row(
          children: [
            Expanded(child: _buildDesktopInputBlock("JENIS KELAMIN", DropdownButtonFormField<String>(
              isExpanded: true, dropdownColor: AppColors.slate800, initialValue: _regJenisKelamin, icon: const Icon(Icons.keyboard_arrow_down, color: Colors.white54), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13), decoration: _desktopInputDeco(""),
        ...
        Row(
          children: [
            Expanded(child: _buildDesktopInputBlock("AGAMA", DropdownButtonFormField<String>(
              isExpanded: true, dropdownColor: AppColors.slate800, initialValue: _regAgama, icon: const Icon(Icons.keyboard_arrow_down, color: Colors.white54), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13), decoration: _desktopInputDeco(""),
        ...
        Row(
          children: [
            Expanded(child: _buildDesktopInputBlock("DEPARTEMEN / DIVISI", DropdownButtonFormField<String>(
              isExpanded: true, dropdownColor: AppColors.slate800, 
              initialValue: _departemens.contains(_regDepartemen) ? _regDepartemen : (_departemens.isNotEmpty ? _departemens.first : null),
            ...
            Expanded(child: _buildDesktopInputBlock("JABATAN / POSISI", DropdownButtonFormField<String>(
              isExpanded: true, dropdownColor: AppColors.slate800, 
              initialValue: _jabatans.contains(_regJabatan) ? _regJabatan : (_jabatans.isNotEmpty ? _jabatans.first : null),
              icon: const Icon(Icons.keyboard_arrow_down, color: Colors.white54), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13), decoration: _desktopInputDeco(""),
        ...
        _buildDesktopInputBlock("AREA PENUGASAN", DropdownButtonFormField<String>(
          isExpanded: true, dropdownColor: AppColors.slate800, 
          initialValue: _availableAreas.contains(_selectedArea) ? _selectedArea : (_availableAreas.isNotEmpty ? _availableAreas.first : null), 
          icon: const Icon(Icons.keyboard_arrow_down, color: Colors.white54), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13), decoration: _desktopInputDeco(""),
      ...
      child: SingleChildScrollView(
        child: Container(
          ...
          child: _step == 'select' 
              ...
              : (_step == 'register' ? _buildMobileRegisterForm() : _buildMobileLoginForm()),
        ),
      ),
    );
  }
  ...
  Widget _buildMobileRoleSelection() {
    return Column(
      ...
      children: [
        ...
        _mobileRoleButton('KARYAWAN', Icons.person, AppColors.yellow500, () => setState(() { _mode = 'karyawan'; _step = 'form'; })),
        const SizedBox(height: 16),
        _mobileRoleButton('IT SUPPORT', Icons.security, AppColors.rose500, () => setState(() { _mode = 'admin'; _step = 'form'; })),
      ],
    );
  }

  Widget _mobileRoleButton(String title, IconData icon, Color hoverColor, VoidCallback onTap) {
    return InkWell(
      onTap: onTap, borderRadius: BorderRadius.circular(24),
      splashColor: hoverColor.withValues(alpha: 0.2), highlightColor: hoverColor.withValues(alpha: 0.1),
      child: Container(
        padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 20),
        decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.03), borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.white.withValues(alpha: 0.08))),
        child: Row(
          children: [
            Icon(icon, color: hoverColor, size: 20),
            const SizedBox(width: 16),
            Expanded(
              child: Text(title.toUpperCase(), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w900, fontSize: 11, letterSpacing: 2)),
            ),
            Icon(Icons.keyboard_arrow_right, color: AppColors.slate500.withValues(alpha: 0.5), size: 18),
          ],
        ),
      ),
    );
  ...
  Widget _buildMobileLoginForm() {
    Color themeColor = _mode == 'admin' ? AppColors.rose500 : _mode == 'head' ? AppColors.blue500 : AppColors.yellow500;
    ...
    return Column(
      ...
      children: [
          ...
          child: Container(
            ...
            child: kIsWeb 
              ? Image.network(
                  'UNTR.JK-97580c63.png', 
                  fit: BoxFit.contain,
                  errorBuilder: (context, error, stackTrace) => Icon(Icons.verified_user, size: 60, color: _mode == 'karyawan' ? AppColors.slate900 : Colors.white),
                )
              : Image.asset(
                  'web/UNTR.JK-97580c63.png', 
                  fit: BoxFit.contain,
        ...
        // FORM FIELDS
        if (_mode == 'karyawan') ...[
          Text("AREA PENUGASAN", style: TextStyle(color: themeColor, fontSize: 9, letterSpacing: 2, fontWeight: FontWeight.bold)),
          const SizedBox(height: 8),
          DropdownButtonFormField<String>(
            isExpanded: true, dropdownColor: AppColors.slate800, 
            initialValue: _availableAreas.contains(_selectedArea) ? _selectedArea : (_availableAreas.isNotEmpty ? _availableAreas.first : null),
            ...
            decoration: InputDecoration(
              ...
              focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(24), borderSide: BorderSide(color: themeColor, width: 2)),
            ),
            onChanged: (String? newValue) => setState(() => _selectedArea = newValue!),
          ...
          builder: (context, constraints) => RawAutocomplete<String>(
            ...
            fieldViewBuilder: (c, t, f, o) {
                  return TextField(
                    ...
                    textInputAction: TextInputAction.next, onSubmitted: (_) => _passFocusNode.requestFocus(),
                    decoration: InputDecoration(
                      labelText: _mode == 'head' ? 'ID ADMIN AREA' : _mode == 'admin' ? 'ID IT SUPPORT' : 'NRP KARYAWAN', labelStyle: TextStyle(color: themeColor, fontSize: 10, letterSpacing: 2, fontWeight: FontWeight.bold),
                      filled: true, fillColor: AppColors.slate900.withValues(alpha: 0.5), contentPadding: const EdgeInsets.symmetric(horizontal: 24, vertical: 18),
                      enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(24), borderSide: BorderSide(color: Colors.white.withValues(alpha: 0.05))),
                      focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(24), borderSide: BorderSide(color: themeColor, width: 2)),
                    ),
        ...
        TextField(
          ...
          textInputAction: TextInputAction.done, onSubmitted: (_) => _handleManualLogin(),
          decoration: InputDecoration(
            labelText: 'KATA SANDI', labelStyle: TextStyle(color: themeColor, fontSize: 10, letterSpacing: 2, fontWeight: FontWeight.bold),
            filled: true, fillColor: AppColors.slate900.withValues(alpha: 0.5), contentPadding: const EdgeInsets.symmetric(horizontal: 24, vertical: 18),
            enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(24), borderSide: BorderSide(color: Colors.white.withValues(alpha: 0.05))),
            focusedBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(24), borderSide: BorderSide(color: themeColor, width: 2)),
            suffixIcon: IconButton(
        ...
        if (_mode != 'admin')
          Align(
            ...
            child: TextButton(
              onPressed: _showForgotPasswordDialog,
              style: TextButton.styleFrom(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4), minimumSize: Size.zero),
              child: Text("Lupa Kata Sandi?", style: TextStyle(color: themeColor, fontSize: 10, fontWeight: FontWeight.bold)),
            ),
          ),
        ...
        SizedBox(
          width: double.infinity, height: 60,
          child: ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: themeColor, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24)), elevation: 10, shadowColor: themeColor.withValues(alpha: 0.3)),
            onPressed: _handleManualLogin,
            child: Text("AKSES SISTEM", style: TextStyle(color: _mode == 'karyawan' ? AppColors.slate900 : Colors.white, fontWeight: FontWeight.w900, letterSpacing: 2, fontSize: 12)),
          ),
        ),
        ...
        if (_mode == 'karyawan' || _mode == 'head') ...[
           ...
           Center(
              child: TextButton(
                 onPressed: () => setState(() { _step = 'register'; _errorMsg = ''; }), 
                 child: Text("DAFTAR SEBAGAI ${_mode == 'head' ? 'ADMIN AREA' : 'KARYAWAN BARU'}", style: const TextStyle(color: AppColors.slate400, fontWeight: FontWeight.w900, fontSize: 10, letterSpacing: 1, decoration: TextDecoration.underline))
              )
           )
        ]
      ],
  ...
  Widget _buildMobileRegisterForm() {
    return Column(
      ...
      children: [
        ...
        Center(child: Text(_mode == 'head' ? "REGISTRASI ADMIN AREA" : "REGISTRASI KARYAWAN", style: const TextStyle(color: Colors.white, fontSize: 20, fontWeight: FontWeight.w900, letterSpacing: -0.5))),
        const SizedBox(height: 8),
        ...
        _buildInputColDark("JENIS KELAMIN", DropdownButtonFormField<String>(
          isExpanded: true, dropdownColor: AppColors.slate800,
          initialValue: _regJenisKelamin, icon: const Icon(Icons.keyboard_arrow_down, color: Colors.white54), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold), decoration: _inputDecoDark(""),
          items: ['Laki-laki', 'Perempuan'].map((e) => DropdownMenuItem(value: e, child: Text(e, overflow: TextOverflow.ellipsis))).toList(),
          onChanged: (v) => setState(() => _regJenisKelamin = v!),
        )),
        const SizedBox(height: 16),
        ...
        _buildInputColDark("AGAMA", DropdownButtonFormField<String>(
          isExpanded: true, dropdownColor: AppColors.slate800,
          initialValue: _regAgama, icon: const Icon(Icons.keyboard_arrow_down, color: Colors.white54), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold), decoration: _inputDecoDark(""),
          items: ['Islam', 'Kristen', 'Katolik', 'Hindu', 'Buddha', 'Konghucu'].map((e) => DropdownMenuItem(value: e, child: Text(e, overflow: TextOverflow.ellipsis))).toList(),
          onChanged: (v) => setState(() => _regAgama = v!),
        )),
        const SizedBox(height: 16),
        _buildInputColDark("NO HANDPHONE", TextField(textInputAction: TextInputAction.next, keyboardType: TextInputType.phone, controller: _regKontakController, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold), decoration: _inputDecoDark("No HP Aktif"))),
        const SizedBox(height: 16),
        ...
        const SizedBox(height: 16),
        _buildInputColDark("AREA PENUGASAN", DropdownButtonFormField<String>(
          isExpanded: true, dropdownColor: AppColors.slate800,
          initialValue: _availableAreas.contains(_selectedArea) ? _selectedArea : (_availableAreas.isNotEmpty ? _availableAreas.first : null),
          icon: const Icon(Icons.keyboard_arrow_down, color: Colors.white54), style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold), decoration: _inputDecoDark(""),
          items: _availableAreas.toSet().map((e) => DropdownMenuItem(value: e, child: Text(e, overflow: TextOverflow.ellipsis))).toList(),
